document.addEventListener('DOMContentLoaded', function () {
  ...
  // --- STATE ---
  let products = [];
  let filters = { categories: {}, productMap: {}, featureCategories: [], gloveGroups: {} };
  ...
  const els = {
    ...
    btnGloveGroups: get('btnGloveGroups'),
    gloveGroupModal: get('gloveGroupModal'),
    gloveGroupModalClose: get('gloveGroupModalClose'),
    gloveGroupManagerList: get('gloveGroupManagerList'),
    btnSaveGloveGroups: get('btnSaveGloveGroups')
  };
    ...
    try {
      ...
      if (storedP && storedF) {
        ...
        filters = {
          ...
          gloveGroups: JSON.parse(JSON.stringify(baseF.gloveGroups || {}))
        };
      }
  ...
  function exportData() {
    ...
    const cleanFilters = {
        ...
        gloveGroups: filters.gloveGroups || {}
    };
...
`;
  ...
  // --- GLOVE GROUP SETTINGS ---
  const DEFAULT_GLOVE_GROUPS = ['Coated', 'Leather', 'Disposable'];

  function openGloveGroupSettings() {
    renderGloveGroupManager();
    if (els.gloveGroupModal) els.gloveGroupModal.classList.add('open');
  }

  function renderGloveGroupManager() {
    const container = els.gloveGroupManagerList;
    if (!container) return;
    container.innerHTML = '';

    // All available filter categories.
    // 'Industry' is excluded because it is used only in the Search by Industry tab.
    // 'Brand' is handled separately below as a per-group brand restriction.
    const GROUP_EXCLUDED_CATS = ['Industry'];
    const allFilterCats = Object.keys(filters.categories || {})
      .filter(c => !GROUP_EXCLUDED_CATS.includes(c))
      .sort();
    const allBrands = (filters.categories['Brand'] || []).slice().sort();

    // Ensure all default groups exist in filters.gloveGroups
    if (!filters.gloveGroups) filters.gloveGroups = {};
    DEFAULT_GLOVE_GROUPS.forEach(groupName => {
      if (!filters.gloveGroups[groupName]) {
        filters.gloveGroups[groupName] = { visibleFilters: [], brands: [] };
      }
    });

    const groupNames = Object.keys(filters.gloveGroups);
    groupNames.forEach(groupName => {
      const groupCfg = filters.gloveGroups[groupName];
      const panel = document.createElement('div');
      panel.style.cssText = 'border:1px solid #e5e7eb; border-radius:10px; margin-bottom:16px; overflow:hidden;';

      // Panel header
      const panelHeader = document.createElement('div');
      panelHeader.style.cssText = 'background:#f9fafb; padding:12px 16px; font-weight:700; font-size:14px; color:#111827; border-bottom:1px solid #e5e7eb;';
      panelHeader.textContent = groupName;
      panel.appendChild(panelHeader);

      const panelBody = document.createElement('div');
      panelBody.style.cssText = 'padding:16px;';

      // Visible Filters section
      const filtersTitle = document.createElement('div');
      filtersTitle.className = 'filter-group-title';
      filtersTitle.textContent = 'Visible Filters';
      panelBody.appendChild(filtersTitle);

      const filtersGrid = document.createElement('div');
      filtersGrid.className = 'check-grid';
      allFilterCats.forEach(cat => {
        const label = document.createElement('label');
        label.className = 'check-label';
        const cb = document.createElement('input');
        cb.type = 'checkbox';
        cb.dataset.group = groupName;
        cb.dataset.type = 'filter';
        cb.dataset.value = cat;
        cb.checked = Array.isArray(groupCfg.visibleFilters) && groupCfg.visibleFilters.includes(cat);
      ...
      panelBody.appendChild(filtersGrid);

      // Brands section
      const brandsTitle = document.createElement('div');
      brandsTitle.className = 'filter-group-title';
      brandsTitle.style.marginTop = '14px';
      brandsTitle.textContent = 'Brands for this group';
      panelBody.appendChild(brandsTitle);

      const brandsGrid = document.createElement('div');
      brandsGrid.className = 'check-grid';
      allBrands.forEach(brand => {
        ...
        cb.type = 'checkbox';
        cb.dataset.group = groupName;
        cb.dataset.type = 'brand';
        cb.dataset.value = brand;
        cb.checked = Array.isArray(groupCfg.brands) && groupCfg.brands.includes(brand);
        const span = document.createElement('span');
        span.textContent = brand;
        label.appendChild(cb);
        label.appendChild(span);
        brandsGrid.appendChild(label);
      });
  ...
  function saveGloveGroupSettings() {
    const container = els.gloveGroupManagerList;
    if (!container) return;
    ...
    saveToStorage();
    showStatus('Glove group settings saved.');
    closeModal();
  }
  ...
  if(els.gloveGroupModalClose) els.gloveGroupModalClose.onclick = closeModal;

  if(els.btnGloveGroups) {
    els.btnGloveGroups.onclick = function(e) {
      e.preventDefault();
      openGloveGroupSettings();
    };
  }

  if(els.btnSaveGloveGroups) els.btnSaveGloveGroups.onclick = saveGloveGroupSettings;
